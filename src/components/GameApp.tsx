              ...
              {isLoading ? (
                ...
                <>
                  ...
                  {view === 'result' && currentSong && (() => {
                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white/10 backdrop-blur-3xl p-6 md:p-8 rounded-[2.5rem] border border-white/20 text-center shadow-2xl max-w-md w-full mx-auto relative z-10 my-4">
                      {gameStatus === 'won' ? (
                        <><CheckCircle size={60} className="text-green-500 mx-auto mb-4" /><h2 className="text-2xl font-black text-white mb-1 uppercase italic">GRATULACJE!</h2><p className={`${currentTheme.text} font-bold text-sm uppercase tracking-widest`}>Zgadłeś w {attempt + 1} próbie</p><p className="text-yellow-500 font-black text-lg animate-bounce mt-1">+{(attempt < 6 ? [100, 80, 60, 40, 20, 10][attempt] : 0) + partialPointsEarned} PKT!</p></>
                      ) : (
                        <><XCircle size={60} className="text-red-500 mx-auto mb-4" /><h2 className="text-2xl font-black text-white mb-1 uppercase italic">KONIEC PRÓB</h2><p className="text-white/40 mb-2 font-bold text-sm uppercase tracking-widest">Wszystkie próby wykorzystane</p>{partialPointsEarned > 0 && <p className="text-yellow-500 font-bold text-lg mb-4">+{partialPointsEarned} PKT</p>}</>
                      )}
